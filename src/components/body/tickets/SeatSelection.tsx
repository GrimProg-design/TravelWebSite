// В components/body/tickets/TicketsComponent/SeatSelection.tsx
...
export default function SeatSelection({ transportType, onComplete, isAnimating }: SeatSelectionProps): JSX.Element {
    const [selectedSeat, setSelectedSeat] = useState<string | null>(null);

    const seats = transportType === 'plane' 
        ? ['A1', 'B1', 'C1', 'D1', 'A2', 'B2', 'C2', 'D2'] // 4x2
        : ['1A', '1B', '2A', '2B', '3A', '3B']; // 2x3

    return (
        <div className={`seat-selection-con ${transportType} ${isAnimating ? 'animate-out' : ''}`}>
            
            <h3 className="stage-header">
                Выберите место в {transportType === 'plane' ? 'самолете' : 'поезде'}
            </h3>

            {/* Контейнер для схемы (сам самолет или вагон) */}
            ...
            <p className="selected-info">
                {selectedSeat ? `Выбрано место: ${selectedSeat}` : 'Выберите доступное место'}
            </p>
